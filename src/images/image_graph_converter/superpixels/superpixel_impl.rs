...
pub fn segment(pixel_map: &PixelMap, num_patches: usize, compactness: f32) -> (usize, Vec<i32>) {
    let image: image::RgbImage = pixel_map.pixels().iter().fold(
        ...
        |mut buffer, px| {
            buffer.put_pixel(px.x as u32, px.y as u32, image::Rgb([px.r, px.g, px.b]));
            buffer
        },
    );

    let mut lab_color = pixel_map
        .pixels()
        .iter()
        .map(|px| {
            let rgb = RGBColor {
            ...
            let mut lab = LABColor::default();
            rgb_to_lab(&rgb, &mut lab);
            lab
        })
        .collect::<Vec<_>>();
    ...
    let num_superpixels = do_segmentation_with_num_superpixels(
        &image,
        &mut lab_color,
        pixel_map.width as u32,
        pixel_map.height as u32,
        &mut klabels,
        num_patches as u32,
        compactness as f64,
        iterations,
...
pub fn rgb_to_lab_image<I: GenericImage>(img: &I, lab_vec: &mut Vec<LABColor>) {
...
pub fn save_image_with_segment_boundaries(
    ...
    for x in 0..width {
        for y in 0..height {
            let idx = y * width + x;
            // only if y > 0
            let up_idx = if y > 0 { (y - 1) * width + x } else { idx };

            // only if x > 0
            let left_idx = if x > 0 { y * width + (x - 1) } else { idx };
...
mod test {
    ...
    fn almost_eq(a: f64, b: f64) {
        ...
        assert!(
            diff < 0.0001,
            format!("{} and {} are not within 0.0001", a, b)
        );
    }

    #[test]
    fn test_rgb_to_xyz() {
        let rgb = super::RGBColor {
            r: 255,
            g: 255,
            b: 22,
        };
